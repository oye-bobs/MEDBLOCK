...
import { Patient, Practitioner, ConsentRecord, ConsentStatus } from '../database/entities';
...
export class PatientsService {
     ...
     */
    async getPatientByDidForProvider(patientDid: string, providerDid: string): Promise<Patient> {
        const provider = await this.practitionerRepo.findOne({ where: { did: providerDid } });
        if (!provider) {
            throw new NotFoundException('Provider not found');
        }
     ...
     */
    async hasProviderAccess(patientDid: string, providerDid: string): Promise<boolean> {
        const provider = await this.practitionerRepo.findOne({ where: { did: providerDid } });
