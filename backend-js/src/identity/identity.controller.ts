...
export class IdentityController {
    ...
    private async createPractitionerAccount(dto: CreatePractitionerDto) {
        ...
        const practitioner = this.practitionerRepository.create({
            ...
            active: false, // Must be verified by admin
    ...
    async loginPractitioner(@Body() body: { email: string; password: string }) {
        ...
        if (!practitioner.active) {
            throw new HttpException('Your account is pending verification by the administrator.', HttpStatus.FORBIDDEN);
        }

        // Verify password
        const storedPassword = practitioner.meta?.password;
        if (!storedPassword) {
            throw new HttpException('Password not set for this account', HttpStatus.UNAUTHORIZED);
        }
